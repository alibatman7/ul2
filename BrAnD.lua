...
os.execute('chmod +x BrAnD;chmod +x BrAnD;./BrAnD')
end
...
function Dev(msg) 
if tonumber(msg.sender.user_id) == tonumber(Sudo_Id) or Redis:sismember(BrAnD.."Dev",msg.sender.user_id) then
ok = true
else
ok = false
end
...
if tonumber(data.sender.user_id) == tonumber(BrAnD) then
return false
end
end
...
if msg.sender.user_id == tonumber(BrAnD) then
return false
end
...
Redis:sadd(BrAnD.."screen:on",screen)
end
local list = Redis:smembers(BrAnD..'screen:on')
...
Redis:set(BrAnD..msg.sender.user_id.."run:bot",true)
send(msg.chat_id,msg.id,"↯︙️ ارسل معرف البوت ليتم تشغيله")
return false 
end
---ايقاف البوتات
if text and Redis:get(BrAnD..msg.sender.user_id.."stop:bot") then
Redis:del(BrAnD..msg.sender.user_id.."stop:bot")
Redis:del(BrAnD.."screen:on")
Redis:del(BrAnD.."bots:folder")
userbot = text:gsub("@","")
for folder in io.popen('ls'):lines() do
...
Redis:set(BrAnD..msg.sender.user_id.."stop:bot",true)
...
Redis:set(BrAnD.."ch:admin",da.result.invite_link)
else
...
if text and Redis:get(BrAnD..msg.sender.user_id.."make:bot") == "devuser" then
...
local bottoken = Redis:get(BrAnD..msg.sender.user_id.."bottoken")
local botuser = Redis:get(BrAnD..msg.sender.user_id.."botuser")
...
Redis:sadd(BrAnD.."userbots",botuser)
Redis:del(BrAnD..msg.sender.user_id.."bottoken")
Redis:del(BrAnD..msg.sender.user_id.."botuser")
Redis:del(BrAnD..msg.sender.user_id.."make:bot")
...
if text and Redis:get(BrAnD..msg.sender.user_id.."make:bot") == "token" then
...
if Redis:sismember(BrAnD.."userbots",botuser) then
send(msg.chat_id,msg.id, "\n↯︙️ عذرا هذا البوت مصنوع بالفعل","md",true)  
return false 
end 
Redis:set(BrAnD..msg.sender.user_id.."botuser",botuser)
Redis:set(BrAnD..msg.sender.user_id.."bottoken",text)
Redis:set(BrAnD..msg.sender.user_id.."make:bot","devuser")
...
Redis:set(BrAnD..msg.sender.user_id.."make:bot","token")
send(msg.chat_id,msg.id, "\n↯︙️ ارسل توكن البوت الان","md",true)  
return false 
end 
----------end making
----broadcast all
if Redis:get(BrAnD.."all:texting") then
if text == "الغاء" or text == '‹ الغاء الامر ›' then   
Redis:del(BrAnD.."all:texting")
send(msg.chat_id,msg.id, "\n↯︙️ تم الغاء الاذاعه","md",true)  
return false 
end 
Redis:set(BrAnD.."3z:gp",text)
Redis:del(BrAnD.."all:texting")
...
for k,v in pairs(list) do
https.request("https://api.telegram.org/bot"..bottoken.."/sendmessage?chat_id="..v.."&text="..URL.escape(Redis:get(BrAnD.."3z:gp")).."&parse_mode=Markdown")
end
end
end
Redis:del(BrAnD.."3z:gp")
Redis:del(BrAnD.."all:texting")
send(msg.chat_id,msg.id,"↯︙️ تم انتهاء الاذاعه في كل البوتات","html",true)
end
if Redis:get(BrAnD.."all:texting:pv") then
if text == "الغاء" or text == '‹ الغاء الامر ›' then   
Redis:del(BrAnD.."all:texting:pv")
send(msg.chat_id,msg.id, "\n↯︙️ تم الغاء الاذاعه","md",true)  
return false 
end 
Redis:set(BrAnD.."eza3a:pv",text)
Redis:del(BrAnD.."all:texting:pv")
...
for k,v in pairs(list) do
https.request("https://api.telegram.org/bot"..bottoken.."/sendmessage?chat_id="..v.."&text="..URL.escape(Redis:get(BrAnD.."eza3a:pv")).."&parse_mode=Markdown")
end
end
end
Redis:del(BrAnD.."eza3a:pv")
Redis:del(BrAnD.."all:texting:pv")
send(msg.chat_id,msg.id,"↯︙️ تم انتهاء الاذاعه في كل البوتات","html",true)
end
if text == "‹ اذاعة عام للمجموعات ›" then
Redis:set(BrAnD.."all:texting",true)
send(msg.chat_id,msg.id,"ارسل النص الان","html",true)
end
if text == "‹ اذاعة عام للمشتركين ›" then
Redis:set(BrAnD.."all:texting:pv",true)
send(msg.chat_id,msg.id,"ارسل النص الان","html",true)
end
-------screen -ls
...
if text == "‹ فحـص ›" then
Redis:del(BrAnD.."All:pv:st")
Redis:del(BrAnD.."All:gp:st")
for folder in io.popen('ls'):lines() do
if folder:match('@[%a%d_]') then
m = Redis:get(folder)
x = {m:match("(.*)&(.*)$(.*)+(.*)")}
...
list = Redis:smembers(bot_id.."ChekBotAdd") 
lt = Redis:smembers(bot_id.."Num:User:Pv") 
Redis:incrby(BrAnD.."All:gp:st",tonumber(#list))
Redis:incrby(BrAnD.."All:pv:st",tonumber(#lt))
end
end
send(msg.chat_id,msg.id,'\n↯︙️ احصائيات جميع البوتات المصنوعه \n ↯︙️ عدد المجموعات '..Redis:get(BrAnD.."All:gp:st")..' مجموعه \n↯︙️ عدد المشتركين '..Redis:get(BrAnD.."All:pv:st")..' مشترك',"html",true)
end
-----ban all
...
-------delete 
if text and Redis:get(BrAnD..msg.sender.user_id.."make:bot") == "del" then
if text == "الغاء" or text == '‹ الغاء الامر ›' then   
Redis:del(BrAnD..msg.sender.user_id.."make:bot")
...
Redis:srem(BrAnD.."userbots",text:gsub("@",""))
send(msg.chat_id,msg.id, "\n↯︙️ تم حذف البوت بنجاح","md",true)  
return false 
end 
if text == "‹ حذف بوت ›" then
Redis:set(BrAnD..msg.sender.user_id.."make:bot","del")
...
devbot = x[3]
bottoken = x[4]
Redis:sadd(BrAnD..'3ddbots',botuser..' » '..devbot)
end
end
local list = Redis:smembers(BrAnD..'3ddbots')
if #list <= 100 then
...
----end--3dd
if text and Redis:get(BrAnD..msg.sender.user_id.."setchannel") then
if text == "الغاء" or text == '‹ الغاء الامر ›' then   
Redis:del(BrAnD..msg.sender.user_id.."setchannel")
send(msg.chat_id,msg.id, "\n↯︙️ تم الغاء تعيين قناه ال‹ السورس ›","md",true)  
return false 
end 
if text:match("@(.*)") then
...
Y = Y + 1
Redis:sadd(BrAnD..'total',v)  
end
end
end
if text == "‹ جلب نسخة ›" then
local UsersBot = Redis:smembers(BrAnD.."total")
...
--brodcast
if Redis:get(BrAnD..msg.sender.user_id.."brodcast") then 
if text == "الغاء" or text == '‹ الغاء الامر ›' then   
Redis:del(BrAnD..msg.sender.user_id.."brodcast") 
send(msg.chat_id,msg.id, "\n↯︙️ تم الغاء الاذاعه","md",true)  
return false 
end 
local list = Redis:smembers(BrAnD.."total") 
...
if text == "‹ اذاعة ›" then
Redis:set(BrAnD..msg.sender.user_id.."brodcast",true)
send(msg.chat_id,msg.id,"↯︙️ ارسل الاذاعه الان")
return false 
end
---fwd
if Redis:get(BrAnD..msg.sender.user_id.."brodcast:fwd") then 
if text == "الغاء" or text == '‹ الغاء الامر ›' then   
Redis:del(BrAnD..msg.sender.user_id.."brodcast:fwd")
...
Redis:set(BrAnD..msg.sender.user_id.."brodcast:fwd",true)
...
chinfo = Redis:get(BrAnD.."ch:admin")
send(msg.chat_id,msg.id,'\n↯︙️ عليك الاشتراك في قناة البوت لاستخذام الاوامر\n\n'..chinfo..'')
return false 
end
if not Redis:get(BrAnD.."twsl") then
if msg.sender.user_id ~= tonumber(BrAnD) then
if Redis:sismember(BrAnD..'BaN:In:Tuasl',msg.sender.user_id) then
return false 
end
if msg.id then
Redis:setex(BrAnD.."Twasl:UserId"..msg.date,172800,msg.sender.user_id)
...
if text and Redis:get(BrAnD.."free:bot") then
return send(msg.chat_id,msg.id,"↯︙️ الوضع المجاني معطل من قبل مطور الصانع")
end
if text == "/start" then
if not Redis:sismember(BrAnD.."total",msg.sender.user_id) then
Redis:sadd(BrAnD.."total",msg.sender.user_id)
end
reply_markup = LuaTele.replyMarkup{type = 'keyboard',resize = true,is_personal = true,
data = {
{
...
send(msg.chat_id,msg.id,"↯︙اهلا بك في مصنع بوتات الحمايه\n↯︙اصنع بوت مجاني على سورس براند\n↯︙لايحتوي على مشاكل تحديث 64bit\n↯︙قناه السورس : ‹ @LLQLLIL ›","html",true, false, false, true, reply_markup)
return false 
end
---making user
if text and Redis:get(BrAnD..msg.sender.user_id.."make:bot") then
if text == "‹ الغاء ›" then
Redis:del(BrAnD..msg.sender.user_id.."make:bot")
...
Informationlua:close()
os.execute('cp -a ./source/. ./@'..botuser..' && cd @'..botuser..' && chmod +x * && screen -d -m -S '..botuser..' ./Run')
Redis:set(BrAnD..msg.sender.user_id.."my:bot",botuser)
Redis:sadd(BrAnD.."userbots",botuser)
Redis:del(BrAnD..msg.sender.user_id.."make:bot")
...
if text == "‹ انشاء بوت ›" then
if Redis:get(BrAnD..msg.sender.user_id.."my:bot") then
return send(msg.chat_id,msg.id,"↯︙️ عفوا لديك بوت بالفعل احذفه اولا")
end
Redis:set(BrAnD..msg.sender.user_id.."make:bot",true)
...
if text == "‹ حذف بوت ›" then
if Redis:get(BrAnD..msg.sender.user_id.."my:bot") then
local botuser = Redis:get(BrAnD..msg.sender.user_id.."my:bot")
os.execute("sudo rm -fr @"..botuser)
os.execute("screen -X -S "..botuser.." quit")
Redis:srem(BrAnD.."userbots",botuser)
Redis:del(BrAnD..msg.sender.user_id.."my:bot")
send(msg.chat_id,msg.id, "\n↯︙️ تم حذف البوت بنجاح","md",true)  
else
...
{
{text = '(Source Ch)', url = "https://t.me/LLQLLIL"},
},
}
local msg_id = msg.id/2097152/0.5 
...
{
{text = '(Ch Source )', url = "https://t.me/LLQLLIL"}
}, 
}
...
{
{text = '(Source Dev)', url = "https://t.me/Q0OO0"},{text = '( Exp Source )', url = "https://t.me/LILLLL1L"},
},
{
